...
- Persist timer changes from both the player and the existing settings selector back into the saved default preference so future sessions reuse the latest chosen duration.
...
- `default-timer-sync`: The player initializes its timer from the saved default for new sessions, and timer changes made in the player or settings stay synchronized with the saved default.
...
- Affected code: `src/favorites/ui/components/FavoriteButton.tsx`, `src/library/ui/components/SoundCard.tsx`, `app/player.tsx`, `app/(tabs)/settings.tsx`, and the existing favorites and UI stores they consume.
