...
- [ ] 1.1 Create the `src/player/ui/components/` slice for `VideoBackground`, `CircularProgressArc`, and `BottomControlPill` so the player UI follows the repository's feature-first structure, reserving `src/player/ui/views/` only for any future screen-scale player views.
...
- [ ] 1.3 Add the minimal unavailable player state with short explanatory copy and a way back to the library when neither route params nor shared playback state resolve to a valid sound.
...
- [ ] 3.4 Wire the player's loop and play or pause interactions to the existing shared audio-store fields and setters without expanding scope into AirPlay, favorites, or broader store API redesign.
