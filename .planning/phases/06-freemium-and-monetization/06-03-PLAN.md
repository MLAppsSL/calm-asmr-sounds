...
If `TimerService` is not in a separate file (it may be inlined in the layout or player), find the countdown logic by reading the current code and add the guard there.
  ...
  <verify>
    ...
    # Verify TimerService guard for infinity mode (check the TimerService file if it exists)
    ls src/services/TimerService.ts 2>/dev/null && grep "timerMode\|infinity" src/services/TimerService.ts || echo "TimerService not in separate file — check layout or player for countdown guard"
    ```
  ...
  <done>
    ...
    - Timer countdown start logic guards on `timerMode === 'infinity'` — no timer starts in infinity mode
