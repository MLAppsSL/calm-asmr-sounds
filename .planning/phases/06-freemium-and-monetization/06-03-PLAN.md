...
When `timerMode === 'infinity'`, the `TimerService` (from Phase 2/3) must NOT start a countdown. Read the TimerService to understand how it currently reads `audioStore` — if it reads `timerSeconds` or `timerStartedAt`, add a guard: `if (timerMode === 'infinity') return;`. If the TimerService is in `src/services/TimerService.ts`, read it and add this guard at the top of the countdown start logic.
...
Also update the TimerService guard: when `timerMode === 'infinity'`, no countdown should start. Find where `TimerService` reads the timer duration and begins countdown; add a guard:

```typescript
// In TimerService (or wherever countdown starts after audio plays):
// Read from audioStore:
const { timerMode } = useAudioStore.getState();
if (timerMode === 'infinity') {
  // Do not start countdown — audio plays indefinitely
  return;
}
// ... existing countdown start logic
```

If `TimerService` is not in a separate file (it may be inlined in the layout or player), find the countdown logic by reading the current code and add the guard there.
  </action>
  <verify>
    ```bash
    ...
    # Verify infinity guard is present in timer countdown logic
    # This MUST return at least one match — the guard that prevents countdown from starting in infinity mode.
    # AUDIO-05 depends entirely on this guard to prevent countdown from cutting off infinite playback.
    grep -rn "infinity" src/ app/ --include="*.ts" --include="*.tsx"
    # Expected: at least one match inside timer countdown logic (TimerService.ts or equivalent)
    ```
    TypeScript must pass with 0 errors. All grep checks return matches. The infinity guard grep MUST return at least one match inside timer countdown logic — zero matches means the guard was never applied and AUDIO-05 is broken.
  </verify>
  <done>
    ...
    - Timer countdown start logic guards on `timerMode === 'infinity'` — no timer starts in infinity mode; `grep -rn "infinity" src/ app/ --include="*.ts" --include="*.tsx"` returns at least one match inside the timer countdown start path
    - `customTimerSeconds` is used when `timerMode === 'countdown'` and not null — replaces the default timer selection
...
# Infinity mode guard in timer logic
...
- Custom timer duration used when set and timerMode is 'countdown'
...
- audioStore: exact new fields and types added, any conflicts with Phase 2/3 timer fields
- TimerService guard: where in the code the infinity guard was added (file + line range)
