...
files_modified:
  - src/shared/data/services/TimerService.ts
  - src/shared/domain/stores/audioStore.ts
  - app/_layout.tsx
  - app/(tabs)/index.tsx
autonomous: false
...
must_haves:
  ...
  artifacts:
    - path: 'src/shared/data/services/TimerService.ts'
      provides: 'AppState-aware countdown that calls AudioService.fadeOut then stop at zero'
      exports: ['TimerService']
      min_lines: 80
    - path: 'src/shared/domain/stores/audioStore.ts'
  ...
  key_links:
    - from: 'src/shared/data/services/TimerService.ts'
      to: 'src/shared/data/services/AudioService.ts'
    ...
    - from: 'src/shared/domain/stores/audioStore.ts'
      to: 'src/shared/data/services/TimerService.ts'
...
Output: `src/shared/data/services/TimerService.ts`, augmented `src/shared/domain/stores/audioStore.ts`, updated `app/_layout.tsx`, test harness `app/(tabs)/index.tsx`, verified on real devices.
</objective>
...
@src/shared/domain/stores/audioStore.ts
@src/shared/domain/types/index.ts
@src/shared/data/services/AudioService.ts
@app/_layout.tsx
</context>
...
<task type="auto">
  ...
  <action>
Augment the existing `src/shared/domain/stores/audioStore.ts` (created in Phase 01-03) by adding timer timestamp fields and control actions. Do NOT rewrite the store — add to it.
...
grep "timerStartedAt" src/shared/domain/stores/audioStore.ts
grep "timerDurationMs" src/shared/domain/stores/audioStore.ts

# Verify new actions exist
grep "startTimer" src/shared/domain/stores/audioStore.ts
grep "resetTimer" src/shared/domain/stores/audioStore.ts
grep "stopTimer" src/shared/domain/stores/audioStore.ts
grep "setTimerDuration" src/shared/domain/stores/audioStore.ts

# Verify existing fields preserved
grep "currentSoundId" src/shared/domain/stores/audioStore.ts
grep "isPlaying" src/shared/domain/stores/audioStore.ts
grep "useAudioStore" src/shared/domain/stores/audioStore.ts
```

  </verify>
  <done>
- `src/shared/domain/stores/audioStore.ts` has `timerStartedAt: number | null` and `timerDurationMs: number` fields
...
<task type="auto">
  ...
  <action>
Create `src/shared/data/services/TimerService.ts` and update `app/_layout.tsx` to initialize both AudioService and TimerService at startup.
...
grep "Date.now()" src/shared/data/services/TimerService.ts
grep "timerStartedAt" src/shared/data/services/TimerService.ts

# Verify AppState listener exists
grep "AppState.addEventListener" src/shared/data/services/TimerService.ts

# Verify fade on timer expiry (LOCKED behavior)
grep "AudioService.fadeOut" src/shared/data/services/TimerService.ts
grep "AudioService.stop" src/shared/data/services/TimerService.ts

# Verify TimerService is a module-level singleton
grep "export const TimerService = new TimerServiceClass" src/shared/data/services/TimerService.ts
...
```
  ...
  <done>
- `src/shared/data/services/TimerService.ts` exports `TimerService` singleton
...
grep "timerStartedAt" src/shared/domain/stores/audioStore.ts
grep "timerDurationMs" src/shared/domain/stores/audioStore.ts
grep "startTimer\|resetTimer\|stopTimer\|setTimerDuration" src/shared/domain/stores/audioStore.ts | wc -l
# Expected: 4
...
- `src/shared/data/services/TimerService.ts` uses timestamp math (`Date.now() - timerStartedAt`) for drift-free countdown
...
- `src/shared/domain/stores/audioStore.ts` has `timerStartedAt: number | null` and `timerDurationMs: 60000 | 120000 | 180000` (union enforces LOCKED timer options); `startTimer`, `resetTimer`, `stopTimer`, `setTimerDuration` actions present; all existing fields preserved
