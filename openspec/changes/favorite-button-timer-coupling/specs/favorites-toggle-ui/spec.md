...
- **THEN** the card SHALL show the filled favorite icon for that sound

#### Scenario: Player reflects current favorite state

- **WHEN** the player screen renders for a sound that is not favorited
- **THEN** the player SHALL show the outlined favorite icon for that sound

### Requirement: Favorite toggle updates immediately

The system SHALL toggle the current sound's favorite state immediately when the user presses the favorite button, and SHALL provide immediate visual feedback for the press interaction.
