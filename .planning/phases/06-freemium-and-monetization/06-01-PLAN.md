...
user_setup:
  ...
  - service: supabase
    ...
    dashboard_config:
      ...
      - task: "Seed the sounds table with premium sound URLs after running the migration — insert one row per premium sound with the audio file URL (Firebase Storage or Supabase Storage URL) and is_premium=true. Free sounds may also be inserted with is_premium=false, or can remain served from the static catalog."
        location: "Terminal: run the seeding SQL via Supabase Studio SQL editor or a one-off migration file"
...
must_haves:
  truths:
    - "A non-premium user cannot access premium sound URLs — the audio files are absent from API responses and cannot be found by inspecting network traffic"
    - "After completing a purchase, premium sounds unlock immediately without restarting the app"
    - "A user who purchases on one session and relaunches the app retains premium status (no re-purchase prompt)"
    - "A user who signs out loses premium status in the UI immediately — premium sounds relock"
    - "A user whose subscription has expired loses access to premium URLs without any client-side change — server-side revocation takes effect on the next Supabase query"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/services/SoundCacheService.ts"
      to: "supabase sounds table"
      via: "supabase.from('sounds').select('url').eq('id', soundId).single() for premium sounds — RLS enforces that non-premium users receive no rows"
      pattern: "from\\('sounds'\\)"
---
...
<task type="auto">
  <name>Task 4: Route premium sound URL resolution through Supabase (SoundCacheService)</name>
  <files>
    src/services/SoundCacheService.ts
  </files>
  <action>
Read the current `src/services/SoundCacheService.ts` (created in Phase 2, Plan 02-02). It currently resolves audio URLs by calling `storage().ref(sound.storageRef).getDownloadURL()` from Firebase Storage for all sounds, both free and premium.

The Phase 6 goal ("Premium sound URLs are never returned to a non-premium user by Supabase RLS") requires that premium sound URLs flow through a Supabase query — not the static catalog or Firebase Storage — so RLS enforcement applies.

**Change to make:** Add a Supabase URL resolution path for premium sounds. When `sound.isPremium === true`, fetch the URL from the Supabase `sounds` table instead of from Firebase Storage directly. When `sound.isPremium === false`, leave the existing Firebase Storage path unchanged.

```typescript
// ADD to SoundCacheService.ts imports:
import { supabase } from '@/src/lib/supabase'; // The Supabase client from Phase 5

// MODIFY the _download private method (or add a new _resolveUrl helper):
// Replace the current getDownloadURL call with a branching resolver:

private async _resolveAudioUrl(sound: SoundConfig): Promise<string | null> {
  if (!sound.isPremium) {
    // Free sounds: use existing Firebase Storage path (unchanged from Phase 2)
    return await storage().ref(sound.storageRef).getDownloadURL();
  }

  // Premium sounds: fetch URL from Supabase sounds table.
  // RLS policy in supabase/migrations/20260219000000_sounds_table_rls.sql
  // enforces that non-premium users receive zero rows — no URL is returned.
  ...
  if (error || !data?.url) {
    // Non-premium user (RLS filtered) or network error — return null.
    // The caller must handle null gracefully (show "Premium required" or route to paywall).
    console.warn(`[SoundCacheService] Supabase URL fetch failed for premium sound ${sound.id}:`, error?.message ?? 'no url returned');
    return null;
  }
...
Update `_download` to call `_resolveAudioUrl(sound)` instead of `storage().ref(sound.storageRef).getDownloadURL()` directly:

```typescript
// In _download (existing method), replace:
//   const downloadUrl = await storage().ref(sound.storageRef).getDownloadURL();
// With:
const downloadUrl = await this._resolveAudioUrl(sound);
if (!downloadUrl) {
  // Premium sound with RLS block (non-premium user) or network error
  return null;
}
```

**Important:** The Supabase client (`supabase`) was initialized with the authenticated user's session in Phase 5. No additional auth configuration is needed — the JWT is automatically attached to Supabase requests via the `@supabase/supabase-js` client's session store. RLS uses `auth.jwt()` to evaluate the policy.

**Note for caller (app/(tabs)/index.tsx or player.tsx):** When `SoundCacheService.getLocalUri()` returns `null` for a premium sound, the caller should call `openPaywall()` instead of attempting playback. The UI already gates premium sounds behind `PremiumGate` (Plan 06-02), so this null return is a defense-in-depth — not the primary gate.

Do NOT cache the `null` result — the user's premium status may change after purchase. Only cache on a successful download (existing behavior: cache hit returns local URI, non-null).
  </action>
  <verify>
    ```bash
    cd /Users/manuelfrancogiraldez/Documents/2026/proyectos/calm-asmr-sounds
    ...
    # Verify Supabase query present in SoundCacheService
    grep "supabase\|from('sounds')\|from(\"sounds\")" src/services/SoundCacheService.ts

    # Verify premium branch exists
    grep "isPremium" src/services/SoundCacheService.ts

    # Verify _resolveAudioUrl or equivalent branching method
    grep "_resolveAudioUrl\|isPremium.*false\|isPremium.*true" src/services/SoundCacheService.ts | head -5

    # Verify existing Firebase Storage path still used for free sounds
    grep "getDownloadURL\|storageRef" src/services/SoundCacheService.ts

    # Verify null handling on Supabase error
    grep "error\|no url\|null" src/services/SoundCacheService.ts | head -5
    ```
    TypeScript must pass with 0 errors. Supabase query present. Firebase Storage path still used for free sounds. Both grep checks return matches.
  </verify>
  <done>
    - `src/services/SoundCacheService.ts` routes premium sound URL resolution through `supabase.from('sounds').select('url').eq('id', sound.id).single()`
    - Free sounds continue to use Firebase Storage `getDownloadURL()` (no change to existing behavior)
    - When Supabase returns no rows (non-premium user blocked by RLS), `_resolveAudioUrl` returns `null` and `_download` returns `null` — no URL is cached or played
    - Supabase client from Phase 5 (`@/src/lib/supabase`) is used — authenticated session JWT is automatically attached to all queries
    - TypeScript passes with 0 errors
  </done>
</task>
...
# SoundCacheService: premium sounds route through Supabase
grep "from('sounds')\|from(\"sounds\")" src/services/SoundCacheService.ts
grep "isPremium" src/services/SoundCacheService.ts

# SoundCacheService: free sounds still use Firebase Storage
grep "getDownloadURL\|storageRef" src/services/SoundCacheService.ts
```
</verification>
...
- `SoundCacheService.ts` routes premium sound URL resolution through `supabase.from('sounds').select('url')` — RLS blocks non-premium users at the database level; free sounds still use Firebase Storage `getDownloadURL()`
- Supabase `sounds` table seeded with premium sound URLs (one row per premium sound with `is_premium=true` and `url` field populated)
...
- SoundCacheService: how the Supabase branching was integrated alongside the existing Firebase Storage path — any import or type conflicts
- Seeding: which premium sound IDs were inserted into the sounds table and whether any URL format differences (Firebase Storage vs Supabase Storage) required adjustments
