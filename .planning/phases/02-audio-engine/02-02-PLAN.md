...
depends_on: []
files_modified:
  - src/shared/data/catalogs/sounds.ts
  - src/shared/data/services/SoundCacheService.ts
...
must_haves:
  ...
  artifacts:
    - path: 'src/shared/data/catalogs/sounds.ts'
      ...
      min_lines: 40
    - path: 'src/shared/data/services/SoundCacheService.ts'
      provides: 'Firebase Storage URL resolution + local caching via expo-file-system'
      exports: ['SoundCacheService']
      min_lines: 60
  key_links:
    - from: 'src/shared/data/services/SoundCacheService.ts'
    ...
    - from: 'src/shared/data/catalogs/sounds.ts'
      to: 'src/shared/domain/types/index.ts'
      via: 'SoundConfig reuses SoundCategory and TimerDuration without extending Sound directly'
...
Output: `src/shared/data/catalogs/sounds.ts` (static catalog) + `src/shared/data/services/SoundCacheService.ts` (URL resolution + cache).
</objective>
...
<task type="auto">
  <name>Task 1: Static sound catalog (src/shared/data/catalogs/sounds.ts)</name>
  <files>
    src/shared/data/catalogs/sounds.ts
  </files>
  <action>
Create `src/shared/data/catalogs/sounds.ts`. This is a pure TypeScript module — no network calls, no React. Following the vertical-slice rule, the shared catalog belongs under `src/shared/data/catalogs/`. It maps sound IDs to Firebase Storage paths plus display metadata.
...
import type { SoundCategory, TimerDuration } from '@/shared/domain/types';
...
grep "export interface SoundConfig" src/shared/data/catalogs/sounds.ts

# Verify at least 6 entries in SOUNDS array

grep -c "id:" src/shared/data/catalogs/sounds.ts

# Expected: 9 (or >= 6)

# Verify no full URLs — only Storage paths

grep "https://" src/shared/data/catalogs/sounds.ts && echo "ERROR: full URLs found" || echo "OK: no full URLs"

# Verify SOUNDS_BY_ID and SOUNDS_BY_CATEGORY exported

grep "export const SOUNDS_BY_ID" src/shared/data/catalogs/sounds.ts
grep "export const SOUNDS_BY_CATEGORY" src/shared/data/catalogs/sounds.ts

````
  </verify>
  <done>
- `src/shared/data/catalogs/sounds.ts` exports `SoundConfig` interface, `SOUNDS` array (9 entries), `SOUNDS_BY_ID` and `SOUNDS_BY_CATEGORY`
...
<task type="auto">
  ...
  <action>
Create `src/shared/data/services/SoundCacheService.ts`. This service checks whether a sound is cached locally first; if not, downloads it from Firebase Storage and saves it to `documentDirectory` (permanent storage, not cacheDirectory which can be cleared by the OS). Add `expo-file-system` to `package.json` if it is not already declared, since this slice depends on `expo-file-system/legacy`.
...
grep "documentDirectory" src/shared/data/services/SoundCacheService.ts
grep "cacheDirectory" src/shared/data/services/SoundCacheService.ts && echo "ERROR: use documentDirectory" || echo "OK"

# Verify legacy import for downloadAsync

grep "expo-file-system/legacy" src/shared/data/services/SoundCacheService.ts

# Verify firebase storage ref usage

grep "getDownloadURL" src/shared/data/services/SoundCacheService.ts

# Verify dedup map

grep "inProgress" src/shared/data/services/SoundCacheService.ts

# Verify module-level singleton export

grep "export const SoundCacheService = new SoundCacheServiceClass" src/shared/data/services/SoundCacheService.ts

# Verify partial-file cleanup on download error

grep "deleteAsync" src/shared/data/services/SoundCacheService.ts

````
  </verify>
  <done>
- `src/shared/data/services/SoundCacheService.ts` exports `SoundCacheService` as a module-level singleton
...
# Catalog integrity
node -e "const s = require('./src/shared/data/catalogs/sounds.ts'); console.log(s.SOUNDS.length)"
# Or check via grep:
grep -c "storageRef:" src/shared/data/catalogs/sounds.ts
# Expected: >= 6

# No full URLs in catalog
grep "https://" src/shared/data/catalogs/sounds.ts && echo "FAIL" || echo "PASS: no raw URLs"

# Permanent cache (documentDirectory, not cacheDirectory)
grep "documentDirectory" src/shared/data/services/SoundCacheService.ts
grep "cacheDirectory" src/shared/data/services/SoundCacheService.ts && echo "FAIL" || echo "PASS"

# Legacy FileSystem import
grep "expo-file-system/legacy" src/shared/data/services/SoundCacheService.ts

# Both singletons exported
grep "export const SoundCacheService" src/shared/data/services/SoundCacheService.ts
grep "export const SOUNDS" src/shared/data/catalogs/sounds.ts
````
...
- `src/shared/data/catalogs/sounds.ts` has 9 sounds across 6 categories, all with `isPremium: false` and Firebase Storage paths (not URLs)
- `SOUNDS_BY_ID` and `SOUNDS_BY_CATEGORY` are exported for O(1) lookup and library rendering
- `src/shared/data/services/SoundCacheService.ts` uses `documentDirectory` exclusively (permanent cache)
