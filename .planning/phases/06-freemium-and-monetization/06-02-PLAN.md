...
must_haves:
  truths:
    ...
    - "Purchase error shows 'Purchase didn't complete. Try again.' — cancellation is silent"
  ...
  try {
    ...
    switch (result) {
      ...
      case PAYWALL_RESULT.CANCELLED:
        // LOCKED DECISION: Silent on cancel — user chose to close
        break;
...
```
  ...
  <done>
    ...
    - Error message is exactly `"Purchase didn't complete. Try again."` (locked decision)
...
For the favorite button on sound cards: the favorite toggle is wired in Phase 4's `FavoriteButton` component. If `FavoriteButton` exists in `src/components/FavoriteButton.tsx`, update it to call `openPaywall()` instead of toggling when `sound.isPremium && !isPremium`. If favorite toggling happens in the screen file, apply the same check there. Read the actual code to determine where the favorite action is invoked.
...
- `openPaywall()` guards fullscreen immersive mode, uses `RevenueCatUI.presentPaywall()`, handles all PAYWALL_RESULT cases with locked-decision behavior
