...
> Correction: Phase 5 auth and cloud sync are standardized on Firebase Auth and Firestore. Supabase-specific wording below is stale planning drift unless and until it is fully rewritten.
...
Purpose: Plans 01 and 02 built the Supabase auth plumbing and sync logic. This plan surfaces them in the UI at the exact locations decided in context: auth section at the bottom of Settings (locked decision), and a subtle nudge on the Favorites screen (Claude's discretion — surfaces auth where the value proposition is highest).
Output: Settings with functional auth section; Favorites with sync nudge; phase verified end-to-end on device.

Note: All `useAuth()` calls connect to the Supabase-based AuthContext from Plan 01. The `user` object is a Supabase `User` with `.id` and `.email` fields.
</objective>
...
The section renders conditionally based on `user` from `useAuth()`. The `user` object is a Supabase User — use `user.email` for display (same field name as Firebase User).
...
cd /Users/manuelfrancogiraldez/Documents/2026/proyectos/calm-asmr-sounds
    ...
    # Verify useAuth is imported from Supabase-based AuthContext (not Firebase)
    grep "from.*context/AuthContext" app/\(tabs\)/settings.tsx app/\(tabs\)/favorites.tsx
    ```
  ...
  <done>
    ...
    - When logged in: `user.email` displayed (Supabase User .email field); "Sign Out" button in red that calls `signOut()`
    ...
    - `useAuth` imported from `@/src/context/AuthContext` (Supabase-based, not Firebase)
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <what-built>
    Complete Phase 5 Auth and Cloud Sync using Supabase Auth:
    - 05-01: Supabase client with AsyncStorage session persistence, AuthContext (supabase.auth API), auth modal screen (/auth)
    - 05-02: FavoritesService (Supabase favorites table read/upsert/migration), useFavoritesSync hook (silent background sync), root layout wired
  ...
  <how-to-verify>
    ...
    **2. Sign up and sign in (AUTH-02) — email/password auth via Supabase:**
    - In Settings: tap "Sign in to sync favorites" → auth modal opens
    - In auth modal: tap "Don't have an account? Sign up"
    ...
    **4. Supabase sync (AUTH-03) — favorites sync across sessions:**
    - With a signed-in account, save 1 new favorite
    - Close and reopen the app
    - Verify: user is still signed in (not prompted to log in again — session persisted via AsyncStorage)
    - Verify: the new favorite is still present (loaded from Supabase favorites table on app open)
    ...
    **6. Cross-device simulation (AUTH-03 deduplication):**
    ...
    - Verify favorites load from Supabase (the account's cloud set)
...
# Auth flow end-to-end: key wiring present (Supabase-based)
grep "useFavoritesSync" app/_layout.tsx
grep "migrateLocalToCloud" src/hooks/useFavoritesSync.ts
grep "from('favorites')\|upsert" src/services/FavoritesService.ts

# Confirm no Firebase references in Phase 5 UI files
grep -rn "firebase\|Firebase\|firestore\|initializeAuth" \
  app/\(tabs\)/settings.tsx app/\(tabs\)/favorites.tsx 2>/dev/null \
  && echo "WARNING: Firebase found" || echo "OK: no Firebase"
```
...
2. Sign up and sign in work with email + password via the auth modal in Settings (Supabase Auth)
3. Favorites saved while anonymous appear in the account after first login (migration ran silently via Supabase upsert)
4. User remains signed in after app restart (session persisted via AsyncStorage in Supabase client)
5. After login, favorites load from Supabase favorites table on next app open
...
- Note on Supabase RLS: confirm that the favorites table RLS policies are configured in the Supabase dashboard (user can only select/insert/update/delete their own rows where user_id = auth.uid())
</output>
